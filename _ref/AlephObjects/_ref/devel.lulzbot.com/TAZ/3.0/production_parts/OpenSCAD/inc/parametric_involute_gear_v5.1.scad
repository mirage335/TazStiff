		...
		union ()
		{
			difference ()
			{
				...
				if (gear_thickness < rim_thickness)
					...
					cylinder (r=rim_radius,h=rim_thickness-gear_thickness+1,$fn=circle_facets*rim_radius);
			}
			if (gear_thickness > rim_thickness)
				cylinder (r=rim_radius,h=gear_thickness);
			if (hub_thickness > gear_thickness)
				translate ([0,0,gear_thickness])
				cylinder (r=hub_diameter/2,h=hub_thickness-gear_thickness,$fn=circle_facets*hub_diameter);
		}
		translate ([0,0,-1])
		cylinder (
			r=bore_diameter/2,
			h=2+max(rim_thickness,hub_thickness,gear_thickness),
			$fn=circle_facets*bore_diameter*3);
		if (circles>0)
		{
			for(i=[0:circles-1])	
				rotate([0,0,i*360/circles])
				translate([circle_orbit_diameter/2,0,-1])
				cylinder(r=circle_diameter/2,h=max(gear_thickness,rim_thickness)+3,$fn=circle_facets*circle_diameter);
		}
	}
}
